...
def fitPeaks(r,guess,left,right,start=-1,end=float('inf'),indWidth=True):
    ...
    use=[i for i in ins if left<r.E[i]<right]
    part1=[i for i in ins if start<=r.E[i]<=left]
    ...
    def peaks(p):
        return sum(peak(abs(p[3*i]),p[3*(i if indWidth else 0)+1],p[3*i+2]) for i in range(len(guess)))
    fullGuess=[]
    for e in guess:
        fullGuess+=[e,1.0,1e5] #assumes width 1 eV and height 1e5 are reasonable
    ...
    return (params,r.E[use],base(params[-1],params[-2])+peaks(params),EError)
